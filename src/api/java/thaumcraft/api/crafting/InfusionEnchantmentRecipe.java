...
public class InfusionEnchantmentRecipe {
    ...
    public InfusionEnchantmentRecipe(
            String research, Enchantment input, int inst, AspectList aspects2, ItemStack[] recipe) {
    ...
    public float getEssentiaMod(ItemStack recipeInput) {
        ...
        while (iterator.hasNext()) {
            int j1 = ((Integer) iterator.next()).intValue();
            if (j1 != enchantment.effectId) mod += EnchantmentHelper.getEnchantmentLevel(j1, recipeInput) * .1f;
        }
        return mod;
    }
}
