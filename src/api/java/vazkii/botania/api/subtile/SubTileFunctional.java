...
public class SubTileFunctional extends SubTileEntity {
    ...
    public void linkPool() {
        ...
        if (linkedPool == null) {
            ...
            if (cachedPoolCoordinates != null
                    && supertile
                            .getWorldObj()
                            .blockExists(
                                    cachedPoolCoordinates.posX,
                                    cachedPoolCoordinates.posY,
                                    cachedPoolCoordinates.posZ)) {
                TileEntity tileAt = supertile
                        .getWorldObj()
                        .getTileEntity(
                                cachedPoolCoordinates.posX, cachedPoolCoordinates.posY, cachedPoolCoordinates.posZ);
        ...
        if (!needsNew) {
            TileEntity tileAt =
                    supertile.getWorldObj().getTileEntity(linkedPool.xCoord, linkedPool.yCoord, linkedPool.zCoord);
    ...
    public boolean bindTo(EntityPlayer player, ItemStack wand, int x, int y, int z, int side) {
        ...
        double dist = (x - supertile.xCoord) * (x - supertile.xCoord)
                + (y - supertile.yCoord) * (y - supertile.yCoord)
                + (z - supertile.zCoord) * (z - supertile.zCoord);
        if (range >= dist) {
