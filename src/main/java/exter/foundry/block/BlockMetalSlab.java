...
public class BlockMetalSlab extends BlockSlab implements ISubBlocks {
    ...
    public boolean onBlockActivated(
        ...
        if ((side == 1 && top || side == 0 && !top)
                && item != null
                && (item.getItem() instanceof ItemBlock && ((ItemBlock) (item.getItem())).field_150939_a == this)
                && item.getItemDamage() == material) {
    ...
    public boolean shouldSideBeRendered(IBlockAccess access, int x, int y, int z, int side) {
        ...
        }
        {
            ...
            return bottom
                    ? (side == 0
                            ? true
                            : (side == 1 && super.shouldSideBeRendered(access, x, y, z, side)
                                    ? true
                                    : !isBlockSingleSlab(access.getBlock(x, y, z))
                                            || (access.getBlockMetadata(x, y, z) & 8) == 0))
                    : (side == 1
                            ? true
                            : (side == 0 && super.shouldSideBeRendered(access, x, y, z, side)
                                    ? true
                                    : !isBlockSingleSlab(access.getBlock(x, y, z))
                                            || (access.getBlockMetadata(x, y, z) & 8) != 0));
        }
