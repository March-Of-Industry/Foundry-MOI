...
public class ContainerMetalCaster extends Container {
    ...
    public ContainerMetalCaster(TileEntityMetalCaster caster, IInventory player_inventory) {
        ...
        for (i = 0; i < 3; ++i) {
            for (j = 0; j < 3; ++j) {
                addSlotToContainer(new SlotCasterMold(
                        te_caster,
                        TileEntityMetalCaster.INVENTORY_MOLD_STORAGE + i * 3 + j,
                        SLOT_STORAGE_X + 18 * j,
                        SLOT_STORAGE_Y + 18 * i));
            }
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slot_index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slot_index >= SLOTS_INVENTORY && slot_index < SLOTS_HOTBAR) {
                ...
                if (!mergeItemStack(
                        stack, SLOTS_TE_MOLD_STORAGE, SLOTS_TE_MOLD_STORAGE + SLOTS_TE_MOLD_STORAGE_SIZE, false)) {
                    return null;
                }
                ...
                SlotCasterMold output_slot =
                        (SlotCasterMold) inventorySlots.get(SLOTS_TE + TileEntityMetalCaster.INVENTORY_MOLD);
