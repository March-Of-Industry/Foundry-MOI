...
public class GuiInductionCrucibleFurnace extends GuiFoundry {
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
        ...
        // Draw heat bar.
        int heat = (te_icf.GetHeat() - TileEntityInductionCrucibleFurnace.HEAT_MIN)
                * HEAT_BAR_WIDTH
                / (TileEntityInductionCrucibleFurnace.HEAT_MAX - TileEntityInductionCrucibleFurnace.HEAT_MIN);
        int melt_point = (te_icf.GetMeltingPoint() - TileEntityInductionCrucibleFurnace.HEAT_MIN)
                * HEAT_BAR_WIDTH
                / (TileEntityInductionCrucibleFurnace.HEAT_MAX - TileEntityInductionCrucibleFurnace.HEAT_MIN);
        if (heat > 0) {
        ...
        if (melt_point > 0) {
            drawTexturedModalRect(
                    window_x + HEAT_BAR_X + melt_point - HEAT_BAR_MELT_WIDTH / 2,
                    window_y + HEAT_BAR_Y,
                    HEAT_BAR_MELT_X,
                    HEAT_BAR_MELT_Y,
                    HEAT_BAR_MELT_WIDTH,
    ...
    public void drawScreen(int mousex, int mousey, float par3) {
        ...
        if (func_146978_c /*isPointInRegion*/(
                HEAT_BAR_X, HEAT_BAR_Y, HEAT_BAR_WIDTH, HEAT_BAR_HEIGHT, mousex, mousey)) {
            List<String> currenttip = new ArrayList<String>();
