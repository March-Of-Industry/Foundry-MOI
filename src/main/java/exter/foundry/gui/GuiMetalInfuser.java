...
public class GuiMetalInfuser extends GuiFoundry {
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
        ...
        if (sub != null && sub.amount > 0) {
            ...
            if (tex != null) {
                ...
                drawTexturedModalRect(
                        window_x + SUBSTANCE_X,
                        window_y + SUBSTANCE_Y + SUBSTANCE_HEIGHT - height,
                        tex.GetX(),
                        tex.GetY() + SUBSTANCE_HEIGHT - height,
                        SubstanceGuiTexture.TEXTURE_WIDTH,
                        height);
                GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    public void drawScreen(int mousex, int mousey, float par3) {
        ...
        if (func_146978_c /*isPointInRegion*/(
                SUBSTANCE_X, SUBSTANCE_Y, SubstanceGuiTexture.TEXTURE_WIDTH, SUBSTANCE_HEIGHT, mousex, mousey)) {
            List<String> currenttip = new ArrayList<String>();
            InfuserSubstance sub = te_infuser.GetSubstance();
            if (sub != null && sub.amount > 0) {
                currenttip.add(StatCollector.translateToLocal("substance." + sub.type));
                currenttip.add(String.valueOf(sub.amount) + " / "
                        + String.valueOf(FoundryAPI.INFUSER_SUBSTANCE_AMOUNT_MAX) + " mL");
            } else {
