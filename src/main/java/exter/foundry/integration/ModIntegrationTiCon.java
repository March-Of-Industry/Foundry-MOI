...
public class ModIntegrationTiCon extends ModIntegration {
    ...
    private static final int INGOT_GCD = GCD(TConstruct.ingotLiquidValue, FoundryAPI.FLUID_AMOUNT_INGOT);
    ...
    private void CreateAlloyRecipe(AlloyMix mix, int index, List<FluidStack> inputs) {
        ...
        if (mapped != null) {
            ...
            in.add(
                    new FluidStack( // Convert TiCon Fluid Stack to Foundry Fluid Stack
                            LiquidMetalRegistry.instance.GetFluid(mapped),
                            ing.amount
                                    * 9
                                    * TConstruct.ingotLiquidValue
                                    / (FoundryAPI.FLUID_AMOUNT_INGOT * INGOT_GCD)));
    ...
    public void OnAfterPostInit() {
        ...
        for (Map.Entry<String, String> e : liquid_map.entrySet()) {
            reverse_liquid_map.put(
                    LiquidMetalRegistry.instance.GetFluid(e.getValue()).getName(), e.getKey());
        }
        ...
        for (ICastingRecipe casting : new ArrayList<ICastingRecipe>(CastingRecipeManager.instance.GetRecipes())) {
            String mapped =
                    reverse_liquid_map.get(casting.GetInputFluid().getFluid().getName());
            if (mapped != null) {
                ...
                if (mapped_fluid != null) {
                    CastingRecipeManager.instance.AddRecipe(
                            ...
                            new FluidStack(
                                    mapped_fluid,
                                    DivCeil(
                                            casting.GetInputFluid().amount * TConstruct.ingotLiquidValue,
            ...
            if (casting.cast != null && !casting.consumeCast) {
                ...
                if (mapped != null) {
                    mapped_liquid = new FluidStack(
                            LiquidMetalRegistry.instance.GetFluid(mapped),
                            DivCeil(
                                    casting.castingMetal.amount * FoundryAPI.FLUID_AMOUNT_INGOT,
                                    TConstruct.ingotLiquidValue));
                }
        ...
        List<tconstruct.library.crafting.CastingRecipe> recipes =
                new ArrayList<tconstruct.library.crafting.CastingRecipe>();
        for (tconstruct.library.crafting.CastingRecipe casting : table_casting.getCastingRecipes()) {
        ...
        for (Map.Entry<String, String> entry : liquid_map.entrySet()) {
            ...
            if (!name.equals("glass.molten")) {
                AtomizerRecipeManager.instance.AddRecipe(
                        "dust" + entry.getValue(),
                        new FluidStack(FluidRegistry.getFluid(name), TConstruct.ingotLiquidValue));
            }
        }
    }
}
