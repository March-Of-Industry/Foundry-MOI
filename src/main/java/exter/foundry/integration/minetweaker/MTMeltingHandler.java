    ...
    public static class MeltingAction extends AddRemoveAction {
        ...
        public String getDescription() {
            return String.format(
                    " %s -> %s",
                    MTHelper.getDescription(recipe.GetInput()), MTHelper.getDescription(recipe.GetOutput()));
        }
    }

    @ZenMethod
    public static void addRecipe(
            ILiquidStack output, IIngredient input, @Optional int melting_point, @Optional int speed) {
        ...
        try {
            recipe = new MeltingRecipe(
                    MTHelper.getIngredient(input), MineTweakerMC.getLiquidStack(output), melting_point, speed);
