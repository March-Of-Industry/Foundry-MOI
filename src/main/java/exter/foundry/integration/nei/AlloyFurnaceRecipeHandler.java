...
public class AlloyFurnaceRecipeHandler extends FoundryRecipeHandler {
    ...
    public void addRecipe(IAlloyFurnaceRecipe recipe) {
        ...
        if (recipe.GetOutput() != null
                && in_a != null
                && in_b != null
                && (!(in_a instanceof OreStack)
                        || OreDictionary.getOres(((OreStack) in_a).name).size() > 0)
                && (!(in_b instanceof OreStack)
                        || OreDictionary.getOres(((OreStack) in_b).name).size() > 0)) {
    ...
    public void loadUsageRecipes(String outputId, Object... results) {
        ...
        if (outputId.equals("item") && results[0] instanceof ItemStack) {
            for (IAlloyFurnaceRecipe recipe : AlloyFurnaceRecipeManager.instance.GetRecipes()) {
                if (FoundryUtils.IsItemMatch((ItemStack) results[0], recipe.GetInputA())
                        || FoundryUtils.IsItemMatch((ItemStack) results[0], recipe.GetInputB())) {
                    addRecipe(recipe);
                }
            }
        }
    ...
    private static void findFuels() {
        ...
        for (ItemStack item : ItemList.items)
            if (!efuels.contains(item.getItem())) {
                ...
                if (burnTime > 0) afuels.add(new FurnaceFuel(item.copy(), burnTime));
            }
    }
