...
public abstract class ItemFirearm extends ItemTool {
    ...
    private static MovingObjectPosition Trace(World world, EntityLivingBase shooter, Entity target, float spread) {
        ...
        if (target != null) {
            dir = Vec3.createVectorHelper(
                            target.posX - start.xCoord, target.posY - start.yCoord, target.posZ - start.zCoord)
                    .normalize();
        } else {
            ...
            dir = Vec3.createVectorHelper(syaw * cpitch, MathHelper.sin(pitch * 0.017453292F), cyaw * cpitch);
        }
        Vec3 vspread = Vec3.createVectorHelper(
                        (random.nextFloat() * 2 - 1), (random.nextFloat() * 2 - 1), (random.nextFloat() * 2 - 1))
                .normalize();
        spread = random.nextFloat() * spread;
        dir = dir.addVector(vspread.xCoord * spread, vspread.yCoord * spread, vspread.zCoord * spread)
                .normalize();
        ...
        List<Entity> entities =
                world.getEntitiesWithinAABBExcludingEntity(shooter, shooter.boundingBox.expand(150, 150, 150));
        double min_dist = obj != null ? obj.hitVec.distanceTo(start) : 150;
        for (Entity ent : entities) {
            if (ent.canBeCollidedWith() && ent.boundingBox != null) {
                MovingObjectPosition ent_obj =
                        ent.boundingBox.expand(0.1, 0.1, 0.1).calculateIntercept(start, end);
                if (ent_obj != null) {
    ...
    public static final void Shoot(
            ItemStack round_item,
            World world,
        ...
        Map<EntityLivingBase, MutablePair<Float, Integer>> entities_hit =
                new HashMap<EntityLivingBase, MutablePair<Float, Integer>>();
            ...
            if (obj != null) {
                switch (obj.typeOfHit) {
                    case BLOCK:
                        ...
                        if (round.BreakGlass(round_item)
                                && b.getMaterial() == Material.glass
                                && b.getBlockHardness(world, obj.blockX, obj.blockY, obj.blockZ) < 0.4) {
                            ...
                            round.OnBulletHitBlock(
                                    ...
                                    obj.blockY,
                                    obj.blockZ,
                                    ForgeDirection.getOrientation(obj.sideHit));
                        }
                        break;
                    case ENTITY:
                        if (obj.entityHit instanceof EntityLivingBase) {
                            Vec3 end =
                                    Vec3.createVectorHelper(obj.entityHit.posX, obj.entityHit.posY, obj.entityHit.posZ);
