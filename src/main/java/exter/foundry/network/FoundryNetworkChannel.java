...
public class FoundryNetworkChannel {
    ...
    private static void WriteTileEntityCoords(ByteBuf data, TileEntity sender) {
    ...
    private static FMLProxyPacket MakeCasterModePacket(TileEntityMetalCaster sender) {
    ...
    private static FMLProxyPacket MakeAtomizerModePacket(TileEntityMetalAtomizer sender) {
        ByteBuf data = Unpooled.buffer();
        WriteTileEntityCoords(data, sender);
        data.writeByte(sender.GetMode().number);
    ...
    private static FMLProxyPacket MakeICFModePacket(TileEntityInductionCrucibleFurnace sender) {
        ByteBuf data = Unpooled.buffer();
        WriteTileEntityCoords(data, sender);
        data.writeByte(sender.GetMode().number);
        return new FMLProxyPacket(data, CHANNEL_NAME);
    ...
    private static FMLProxyPacket MakeAlloyMixerModePacket(TileEntityAlloyMixer sender) {
        ByteBuf data = Unpooled.buffer();
        WriteTileEntityCoords(data, sender);
        data.writeByte(sender.GetMode().number);
        return new FMLProxyPacket(data, CHANNEL_NAME);
    ...
    public void SendCasterModeToClients(TileEntityMetalCaster sender) {
        network_channel.sendToAllAround(
                ...
                new TargetPoint(
                        sender.getWorldObj().provider.dimensionId, sender.xCoord, sender.yCoord, sender.zCoord, 192));
    }
