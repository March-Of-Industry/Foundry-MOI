...
public class AlloyFurnaceRecipe implements IAlloyFurnaceRecipe {
    ...
    public boolean MatchesRecipe(ItemStack in_a, ItemStack in_b) {
        return FoundryUtils.IsItemMatch(in_a, input_a)
                && in_a.stackSize >= FoundryUtils.GetStackSize(input_a)
                && FoundryUtils.IsItemMatch(in_b, input_b)
                && in_b.stackSize >= FoundryUtils.GetStackSize(input_b);
    }
}
