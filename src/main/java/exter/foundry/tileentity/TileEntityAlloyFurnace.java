...
public class TileEntityAlloyFurnace extends TileEntityFoundry implements ISidedInventory, IExoflameHeatable {
    ...
    private void DoSmelt(IAlloyFurnaceRecipe recipe, boolean reversed) {
        ...
        if (inv_output != null
                && (!inv_output.isItemEqual(output)
                        || inv_output.stackSize - output.stackSize > inv_output.getMaxStackSize())) {
            progress = 0;
            return;
        }
    ...
    protected void UpdateEntityServer() {
        ...
        if (inventory[SLOT_INPUT_A] != null && inventory[SLOT_INPUT_B] != null) {
            ...
            if (recipe == null) {
                recipe =
                        AlloyFurnaceRecipeManager.instance.FindRecipe(inventory[SLOT_INPUT_B], inventory[SLOT_INPUT_A]);
                if (recipe != null) {
                    reversed = true;
                }
            }
    ...
    public boolean canSmelt() {
        if (inventory[SLOT_INPUT_A] != null && inventory[SLOT_INPUT_B] != null) {
            IAlloyFurnaceRecipe recipe =
                    AlloyFurnaceRecipeManager.instance.FindRecipe(inventory[SLOT_INPUT_A], inventory[SLOT_INPUT_B]);
            if (recipe == null) {
                recipe =
                        AlloyFurnaceRecipeManager.instance.FindRecipe(inventory[SLOT_INPUT_B], inventory[SLOT_INPUT_A]);
            }
