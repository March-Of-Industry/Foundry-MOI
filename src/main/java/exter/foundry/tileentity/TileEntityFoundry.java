    ...
    private void SendPacketToPlayers(Packet packet) {
        ...
        if (!worldObj.isRemote && packet != null) {
            for (int j = 0; j < worldObj.playerEntities.size(); j++) {
                ...
                if (Math.abs(player.posX - xCoord) <= MAX_DISTANCE
                        && Math.abs(player.posY - yCoord) <= MAX_DISTANCE
                        && Math.abs(player.posZ - zCoord) <= MAX_DISTANCE
                        && player.dimension == worldObj.provider.dimensionId) {
